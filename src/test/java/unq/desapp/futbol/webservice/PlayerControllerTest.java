...
class PlayerControllerTest {
    ...
    class HappyPath {
        ...
        void shouldReturnOkWithPerformanceWhenPlayerFound() {
            ...
            StepVerifier.create(result)
                    .assertNext(response -> {
                        ...
                        assertThat(response.getBody().getSeasons().get(0).season()).isEqualTo("2022/2023");
                    })
                    .verifyComplete();

            verify(footballDataService).getPlayerPerformance(expectedPlayerName);
        }
        ...
        void shouldConvertHyphensToSpacesInPlayerName() {
            ...
            when(footballDataService.getPlayerPerformance(expectedPlayerName))
                    .thenReturn(Mono.just(new PlayerPerformance(expectedPlayerName, Collections.emptyList())));

            // Act
            Mono<ResponseEntity<PlayerPerformance>> result = playerController
                    .getPlayerPerformance(playerNameWithMultipleHyphens);
        ...
        void shouldHandlePlayerNameWithoutHyphens() {
            ...
            PlayerPerformance expectedPerformance = new PlayerPerformance(playerName, Collections.emptyList());

            when(footballDataService.getPlayerPerformance(playerName))
                    .thenReturn(Mono.just(expectedPerformance));

            // Act
            Mono<ResponseEntity<PlayerPerformance>> result = playerController.getPlayerPerformance(playerName);
            ...
            verify(footballDataService).getPlayerPerformance(playerName);
        }
    }
    ...
    class ErrorCases {
        ...
        void shouldReturnNotFoundWhenServiceReturnsEmptyMono() {
            ...
            when(footballDataService.getPlayerPerformance(anyString()))
